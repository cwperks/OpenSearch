...
/**
 * Permissions transport action. Computes the explain on the targeted shard.
 *
 * @opensearch.internal
 */
// TODO: AggregatedDfs. Currently the idf can be different then when executing a normal search with explain.
public class TransportPermissionsAction extends TransportSingleShardAction<PermissionsRequest, PermissionsResponse> {
    ...
    @Override
    protected void doExecute(Task task, PermissionsRequest request, ActionListener<PermissionsResponse> listener) {
        request.nowInMillis = System.currentTimeMillis();
        super.doExecute(task, request, listener);
    }

    @Override
    protected boolean resolveIndex(PermissionsRequest request) {
        return true;
    }
    ...
    @Override
    protected void asyncShardOperation(PermissionsRequest request, ShardId shardId, ActionListener<PermissionsResponse> listener)
        throws IOException {
    ...
    @Override
    protected PermissionsResponse shardOperation(PermissionsRequest request, ShardId shardId) throws IOException {
        ...
        try {
            ...
            if (!result.exists()) {
                return new PermissionsResponse(shardId.getIndexName(), request.id(), false);
            }
            ...
            if (request.storedFields() != null || (request.fetchSourceContext() != null && request.fetchSourceContext().fetchSource())) {
                ...
                return new PermissionsResponse(shardId.getIndexName(), request.id(), true, explanation, getResult);
            } else {
                return new PermissionsResponse(shardId.getIndexName(), request.id(), true, explanation);
            }
        } catch (IOException e) {
    ...
    @Override
    protected Writeable.Reader<PermissionsResponse> getResponseReader() {
        return PermissionsResponse::new;
    }
    ...
    @Override
    protected String getExecutor(PermissionsRequest request, ShardId shardId) {
        IndexService indexService = searchService.getIndicesService().indexServiceSafe(shardId.getIndex());
